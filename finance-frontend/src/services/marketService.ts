...
export const cleanInstrumentDisplayName = (name?: string | null, symbol?: string | null): string => {
    ...
    return raw
        .replace(/T\?RK\?YE/gi, 'TÜRKİYE')
        .replace(/\?\?\s*BANKASI/gi, 'İŞ BANKASI')
        .replace(/T\?RK\?YE\s+İŞ\s+BANKASI/gi, 'TÜRKİYE İŞ BANKASI')
        .replace(/POL\?TEKN\?K/gi, 'POLİTEKNİK')
        .replace(/\?LET\?S\?M/gi, 'İLETİŞİM')
        .replace(/\?LETISIM/gi, 'İLETİŞİM')
        .replace(/TA\?IMACILI\?I/gi, 'TAŞIMACILIĞI')
        .replace(/\?ZDEM\?R/gi, 'ÖZDEMİR')
        .replace(/ENERJ\?/gi, 'ENERJİ')
        .replace(/ELEKTR\?K/gi, 'ELEKTRİK')
        .replace(/RET\?M/gi, 'ÜRETİM')
        .replace(/B\?LG\?SAYAR/gi, 'BİLGİSAYAR')
        .replace(/MALZEMELER\?/gi, 'MALZEMELERİ')
        .replace(/GAYR\?MENKUL/gi, 'GAYRİMENKUL')
        .replace(/ORTAKLI\?I/gi, 'ORTAKLIĞI')
        .replace(/HOLD\?NG/gi, 'HOLDİNG')
        .replace(/YATIRIM\s+HOLD\?NG/gi, 'YATIRIM HOLDİNG')
        .replace(/TUR\?ZM/gi, 'TURİZM')
        .replace(/R\?NLER\?/gi, 'ÜRÜNLERİ')
        .replace(/\bB\?EN\b/gi, 'BİEN')
        .replace(/\b\?Z\b/gi, 'ÖZ')
        .replace(/A\.\?/gi, 'A.Ş')
